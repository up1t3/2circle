...
function way_function(way)
	...
	if highway~="" then
		...
		if minzoom <= 14 then
			...
			-- 2circle addition: surface + smoothness drive the client's road colour
			-- scheme. CRITICAL: must survive into the vector tile attributes.
			local surface = way:Find("surface")
			if surface ~= "" then way:Attribute("surface", surface) end
			local smoothness = way:Find("smoothness")
			if smoothness ~= "" then way:Attribute("smoothness", smoothness) end
